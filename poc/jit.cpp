...
public:
  ...
  instantiate_topologically_ordered_nodes(Context &ctx, Graph const& g) const
  {
    std::vector<std::unique_ptr<CodegenNode>> nodes;
    nodes.reserve(g.vertices.size());

    for (auto const &[i, vertex]: g.vertices | std::views::enumerate) {
